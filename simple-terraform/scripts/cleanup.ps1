...
function Remove-TerraformEnvironment {
    ...
    if (-not (Test-Path $workspace)) {
        Write-Host "  Workspace not found: $workspace" -ForegroundColor Yellow
    ...
    try {
        ...
        if ($confirm -ne "yes") {
            Write-Host "  Skipped $Environment" -ForegroundColor Gray
            Pop-Location
            Write-Host ""
            return
        }
...
function Remove-AzureResourcesForce {
    ...
    Write-Host ""
    Write-Host "  WARNING: This will delete resource groups directly!" -ForegroundColor Yellow
